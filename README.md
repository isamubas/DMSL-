...
[**Agricultural Commodities by Market**](https://www.kaggle.com/datasets/alexneakameni/agricultural-production-in-africa)
(`psd_alldata.csv` — all commodities, ~1.89M rows, 183 MB), which repackages the
upstream [USDA FAS PSD Online](https://apps.fas.usda.gov/psdonline/) database.

> **Note on that link.** The URL slug reads `agricultural-production-in-africa`, but the
> dataset is titled *Agricultural Commodities by Market* and the file is the **global**
> USDA PSD database — 40 countries including the US, Australia and the EU, with the exact
> PSD schema (`Commodity_Code`, `Market_Year`, `Attribute_ID`, `Unit_ID`). It is not
> Africa-specific. Worth stating explicitly, since anyone following that URL expecting
> African data will be confused.
